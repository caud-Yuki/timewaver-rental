          ...
          <Dialog open={isFormOpen} onOpenChange={(open) => { if (!open) { setIsFormOpen(false); setEditingTemplate(null); setShowPlaceholders(false); } }}>
            <DialogContent className={`max-h-[90vh] overflow-y-auto transition-all ${showPlaceholders ? 'sm:max-w-5xl' : 'sm:max-w-3xl'}`}>
              ...
              <div className={`flex gap-4 ${showPlaceholders ? '' : ''}`}>
                ...
                {/* Placeholder sidebar */}
                {showPlaceholders && (
                  <div className="w-[220px] shrink-0 border-l pl-4 py-4 space-y-3 overflow-y-auto max-h-[60vh]">
                    <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-wide">プレースホルダー</h4>
                    <p className="text-[10px] text-muted-foreground">クリックで件名/本文に挿入</p>
                    {[
                      { group: 'ユーザー情報', keys: ['userName', 'userEmail'] },
                      { group: '機器情報', keys: ['deviceType', 'deviceId', 'deviceSerialNumber'] },
                      { group: '申請情報', keys: ['applicationId', 'payAmount', 'payType', 'rentalType'] },
                      { group: '配送先', keys: ['shippingAddress1', 'shippingAddress2', 'shippingZipcode', 'shippingPrefecture', 'shippingTel', 'shippingCompanyName'] },
                      { group: '決済・契約', keys: ['paymentLinkId', 'deliveryDate', 'deadline', 'endDate', 'startAt'] },
                      { group: 'クーポン', keys: ['couponCode', 'couponDiscount', 'originalAmount'] },
                    ].map((section) => (
                      <div key={section.group}>
                        <p className="text-[10px] font-semibold text-muted-foreground mb-1">{section.group}</p>
                        <div className="flex flex-wrap gap-1">
                          {section.keys.map((key) => (
                            <button
                              key={key}
                              type="button"
                              className="text-[10px] px-2 py-1 rounded-md border bg-gray-50 hover:bg-primary/10 hover:border-primary/30 transition-colors cursor-pointer font-mono"
                              onClick={() => {
