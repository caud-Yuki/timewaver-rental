...
- **日付は UTC 起算。** Cloud Functions は UTC で動くため、振込期限・発送バッファの営業日計算と
  メールに差し込む日付は UTC の日付になる。JST 00:00〜09:00 に案内を送ると、JST の感覚より
  1 日前の日付が載る（土日スキップの判定も UTC の曜日）。振込に限らず `addBusinessDays` を
  使う全ての日付に共通の挙動。`tests/bank-transfer.test.mjs` は比較側を UTC にそろえてある。
